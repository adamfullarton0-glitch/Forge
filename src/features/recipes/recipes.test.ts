...
describe('classification helpers', () => {
  ...
  it('bundles a photo for every hand-written featured/meal recipe', () => {
    ...
    // A generated combo is not a named featured photo (it uses the combo set).
    expect(hasRecipePhoto(GEN_RECIPES[0]!.name)).toBe(false);
  });

  it('gives every generated recipe a protein × format combo photo', () => {
    for (const r of GEN_RECIPES) {
      const url = recipePhoto(r);
      expect(url).toMatch(/\/recipes\/gen\/[a-z0-9-]+\.jpg$/);
    }
  });
});
...
describe('safety + gating + filtering', () => {
  ...
  it('recipePhoto resolves featured by name, generated by combo, custom → null', () => {
    ...
    // Generated recipe → its protein × format combo photo.
    expect(recipePhoto(GEN_RECIPES[0]!)).toMatch(/^\/recipes\/gen\/.+\.jpg$/);
    // A custom recipe (no combo indices, not a featured name) → gradient tile.
